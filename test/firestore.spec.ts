...
describe('Test firestore functions:', () => {
  ...
  it('Restore', async () => {
    try {
      const result = await firebaseHelper.firestore.restore(
        db,
  ...
  it('Create document with ID', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.createDocumentWithID(
        db,
        collectionName,
        '1',
        { test: true }
  ...
  it('Create new document without an ID', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.createNewDocument(
        db,
        collectionName,
        { test: true }
      )
  ...
  it('Update a document', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.updateDocument(
        db,
        collectionName,
        '1',
        { test: false }
  ...
  it('Check if document exists: True', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.checkDocumentExists(
        db,
        collectionName,
        'first-key'
      )
  ...
  it('Get a valid document', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.getDocument(
        db,
        collectionName,
        'first-key'
      )
  ...
  it('Query Data without order', async () => {
    ...
    let result
    try {
      result = await firebaseHelper.firestore.queryData(
        db,
        collectionName,
        query
      )
  ...
  it('Query Data with multi pagination', async () => {
    ...
    let result
    try {
      result = await firebaseHelper.firestore.queryDataWithPagiation(
        db,
        collectionName,
        queryArray,
        orderBy,
  ...
  it('Delete a document with document id', async () => {
    let result
    try {
      result = await firebaseHelper.firestore.deleteDocument(
        db,
        collectionName,
        '1'
      )
